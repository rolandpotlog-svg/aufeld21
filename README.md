...
### Handy-Push nur für Julia und Roland

- Ausschließlich neue Meldungen, ausschließlich die beiden freigeschalteten aktiven Admin-Konten. Keine Mieter- oder Rechnungs-Pushs; bestehende E-Mails bleiben unverändert. Keine Nachsendung alter Meldungen.
- Migrationen `20260907193753_admin_web_push.sql` und `20260907195956_push_subscription_ownership_guard.sql` vor dem Anwendungscode ausrollen. Bestehendes `SUPABASE_SECRET_KEY` bleibt ausschließlich auf dem Server. Der erste autorisierte Abruf der Push-Einstellungen erstellt einmalig VAPID-Schlüssel und ein Dispatch-Secret im Vault-Eintrag `aufeld21_web_push_config`; weder Browser noch Logs erhalten private Schlüssel. Nicht rotieren, solange bestehende Geräte verbunden bleiben sollen.
- Unter **Admin → E-Mail & Push → Auf diesem Handy aktivieren** muss jeder selbst die Gerätefreigabe erteilen. iPhone/iPad ab iOS 16.4: Safari → Teilen → Zum Home-Bildschirm, dann das neue Symbol öffnen. Immer die kanonische Domain `https://www.aufeld21.at/portal` verwenden. Anschließend auf jedem Gerät **Test-Push senden**; eine Serverfreigabe allein beweist keine Zustellung auf dem Handy.
- Jeder neue Meldungsdatensatz erzeugt transaktional einen Auftrag pro aktiv verbundenem Gerät. Der Supabase-Cron `aufeld21-admin-push` weckt minütlich die Vercel-Route `/api/cron/push` mit kurzlebigem HMAC. Die Verarbeitung beansprucht Aufträge atomar, prüft die Empfänger erneut und wiederholt höchstens dreimal. Abgelaufene Abonnements werden deaktiviert. Zum betrieblichen Pausieren ausschließlich diesen benannten Cron deaktivieren, nicht den E-Mail-Cron.
- Sperrbildschirm-Inhalt ist nur ein allgemeiner Hinweis; Klick öffnet den geschützten Meldungsbereich. Kein Offline-Cache von Portal-Seiten oder Rechnungen. Der Service Worker merkt sich höchstens 100 anonyme Zustellungs-IDs gegen doppelte Anzeige. Beim Abmelden wird dieses Geräte-Abonnement getrennt.
- Tabellen und Konfigurations-RPCs sind für Browser gesperrt. Nur die serverseitig authentifizierten, namentlich erlaubten Konten können eigene Geräte registrieren, testen und trennen. Höchstens zehn aktive Geräte pro Konto. Andere Admins erhalten diese Freigabe nicht automatisch.
- `sent`/`last_success_at` bedeuten Annahme durch den Pushdienst, nicht Lesen oder garantierte Anzeige. Gerätestatus, Fokusmodus, Netzwerk und Browser können verzögern. Fehlerübersicht serverseitig in `push_notifications` prüfen; niemals Rohfehler mit Abonnementadressen protokollieren. Tests stehen in `lib/push/push.test.ts`.
...
Meldungen, Rechnungen und Unterlagen aktualisieren sich beim Zurückkehren ins Portal sowie minütlich im sichtbaren Tab. Das sind **In-App-Hinweise**; die Lesebestätigung bleibt browserbezogen. Seit dem Update vom 07.09.2026 gibt es zusätzlich optionalen Betriebssystem-Push für neue Meldungen ausschließlich für Julia und Roland (Einrichtung siehe oben). Mieter-Rechnungen erhalten weiterhin In-App- und E-Mail-Hinweise, keinen Handy-Push.
